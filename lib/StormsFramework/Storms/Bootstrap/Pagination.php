 ...
 * @version   3.0.0
 *
 * StormsFramework\Storms\Front\Bootstrap\Pagination class
...
class Pagination
{
	 ...
	 */
	public static function loop_pagination( $args = array() ) {
		...
		$page_links = Pagination::bootstrap_paginate_links( $args );
	 ...
	 */
	public static function bootstrap_paginate_links( $args = '' ) {
		$defaults = array(
		...
		if ( $prev_next && $current && 1 < $current ) :
			...
			$link .= $add_fragment;
			$page_links[] = '<li><a class="prev" rel="prev" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $prev_text . '</a></li>';
		endif;
		...
		for ( $n = 1; $n <= $total; $n++ ) :
			...
			if ( $n == $current ) :
				...
				elseif ( $dots && !$show_all ) :
					$page_links[] = '<li class="disabled dots"><span>' . __( '&hellip;', 'storms' ) . '</span></li>';
					$dots = false;
				endif;
			endif;
		endfor;
		...
		if ( $prev_next && $current && ( $current < $total || -1 == $total ) ) :
			...
			if ( $add_args )
				$link = add_query_arg( $add_args, $link );
			$link .= $add_fragment;
			$page_links[] = '<li><a class="next" rel="next" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $next_text . '</a></li>';
		endif;
