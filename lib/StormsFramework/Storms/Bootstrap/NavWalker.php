 ...
 * Customized by Storms Framework
 *
 * Version: 2.0.4
...
class NavWalker extends \Walker_Nav_Menu
{
	...
	public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
	{
		...
		if (strcasecmp($item->attr_title, 'divider') == 0 && $depth === 1) {
			...
			if ($args->has_children && $depth === 0) {
				// @TODO Incluir uma opção que indique se o theme que tirar ou manter o link no menu pai
				//$atts['href'] = '#';
	...
	public static function fallback($args)
	{
		if (current_user_can('manage_options')) {
			...
			$fb_output .= '>';
			$fb_output .= '<li><a href="' . admin_url('nav-menus.php') . '">'. __( 'Add a menu', 'storms' ) . '</a></li>';
			$fb_output .= '</ul>';
			if ($container)
				$fb_output .= '</' . $container . '>';
			echo $fb_output;
